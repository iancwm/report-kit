...
\begin{diagram}[type=flow,caption={A vertical publication flow.},source={Conceptual diagram.},description={Three ordered steps arranged vertically from request through review to publication.}]
  \begin{reportflow}[direction=vertical]
    \step{vertical-request}{Request}
    \step{vertical-review}{Review}
    \step{vertical-publish}{Publish}
    \flowedge{vertical-request}{vertical-review}
    \flowedge{vertical-review}{vertical-publish}
  \end{reportflow}
\end{diagram}
...
\begin{diagram}[type=tree,caption={Revenue issue tree.},source={Conceptual diagnostic framework.},description={Revenue decomposes into volume and price, each with two child drivers.}]
  \begin{reporttree}[kind=issue]
    ...
    \branch{\rkcode{partition_key}}{Customers, Usage}
