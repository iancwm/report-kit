...
Two independent axes -- effort and impact -- sort candidate initiatives into four
quadrants at a glance, which a bulleted list cannot do without forcing the reader to
hold both axes in their head at once. \texttt{RKMatrix} makes the two axes, and the
resulting quadrant a given item falls into, directly visible.

\begin{diagram}[
  type=matrix,
  caption={Initiatives by effort and impact.},
  label={fig:matrix},
  source={Illustrative placement; not derived from a scored rubric.},
  description={A 2x2 matrix with effort on the horizontal axis and impact on the vertical axis, one initiative labelled in each quadrant.}
]
  \RKMatrixSetup{10.4}{6.2}
  \RKMatrix{Effort}{Impact}
  \RKMatrixCell{nw}{Quick polish}{Low effort, high impact}
  \RKMatrixCell[accent]{ne}{Flagship bet}{High effort, high impact}
  \RKMatrixCell{sw}{Backlog}{Low effort, low impact}
  \RKMatrixCell{se}{Reconsider}{High effort, low impact}
\end{diagram}

\section{Layered architecture}
The system separates concerns into three layers of equal visual weight, distinct from
the matrix above: no axis, just top-to-bottom separation of responsibility.
\texttt{RKLayer} renders each layer as its own labelled band.
...
\begin{diagram}[
  ...
  \RKLayerSetup{10}{1.0}
  \RKLayer[rk accent]{0}{Presentation}{Renders reports and diagrams for the reader.}
  \RKLayer{1}{Application logic}{Applies ReportKit macros and semantic styling.}
  \RKLayer{2}{Data storage}{Holds source content and configuration.}
\end{diagram}
...
\section{Code and output blocks}
No fixture exercised \texttt{codeblock}/\texttt{outputblock} before reportkit-code.sty
v1.2.0 moved their space reservation onto the \texttt{\string\RKReserveSpace} renderer
hook; this section is that coverage.

\begin{codeblock}[figures.py]{Python}
def risk_reward_chart(base, bull, bear):
    return max(bull - base, 0) / max(base - bear, 1)
\end{codeblock}
