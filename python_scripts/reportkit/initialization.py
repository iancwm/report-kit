...
def initialize_project(target: Path, repo_root: Path) -> tuple[Path, list[str]]:
    """Create the standard consumer-project directories and starter files.

    Existing files are never overwritten, making the operation safe to rerun
    after a project has acquired content.
    """
...
def _texmf_local() -> Path:
    ...
    result = subprocess.run(
        [kpsewhich, "-var-value", "TEXMFLOCAL"], capture_output=True, text=True, check=False,
    )
...
def _font_files_resolvable() -> bool:
    ...
    if not kpsewhich:
        return False
    return all(
        subprocess.run([kpsewhich, name], capture_output=True, text=True, check=False).stdout.strip()
...
def install_fonts(repo_root: Path) -> str:
    """Install the bundled Libertinus subset into the local TeX tree.

    Font installation is deliberately opt-in because ``TEXMFLOCAL`` is
    normally a system-owned directory. Existing installations are reused.
    """
    ...
    if mktexlsr:
        result = subprocess.run([mktexlsr, str(texmf_local)], capture_output=True, text=True, check=False)
