...
For a multi-chapter publication that needs parts, appendices, a reference list or a glossary, use `\documentclass[theme=default,publication-type=book]{reportkit}` (`theme=technical` is the same target; `theme=editorial` requires `lualatex`), or set `document.publication_type: book` in `publication.yaml`, where every top-level Markdown heading opens a numbered chapter after the title, publication-details and contents pages. The type is experimental until pinned visual review. Each `\section` is a chapter; the type adds only `\bookpart`, `bookdetails` with `\bookdetail`, `\bookappendix`, `bookreferences` (a `thebibliography` list for `\cite`) and `bookglossary` with `\glossaryterm`, and reuses `\RKTitlePage`, `\RKContents`, `\RKFrontMatterBegin` and `\RKMainMatterBegin`. There is no recto/verso, index or print-production support. `latex_templates/examples/book/` compiles unchanged under `default` and `editorial`.
...
## Executive brief (short decision document)

For a short (2–8 page) management decision document, use `\documentclass[theme=executive,publication-type=executive-brief]{reportkit}` (or `theme=institutional-research`; both require `lualatex`), or set `document.publication_type: executive-brief` in `publication.yaml`. The type is experimental until pinned visual review. Order the brief as recommendation, key findings, compact exhibits, implication, risks, next steps, and sources. Reuse existing primitives: `decisionpoint` for the recommendation and implication, `metric` for headline numbers, `redflag`/`assumption` for risks, and the shared `exhibit`/`exhibitpair`/`exhibitgrid`/`financialtable` system (with `source=`) for evidence. The type adds only `briefheader` with `\briefmeta` (masthead and decision metadata), `briefactions` with `\briefaction` (action, owner, due date), and `briefsources`. Sections flow continuously without page breaks. [latex_templates/examples/executive-brief/](latex_templates/examples/executive-brief/) is a complete worked example that compiles unchanged under both themes.
