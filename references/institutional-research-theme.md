...
| `joinmatch` | `O{} m m` | `options` (options, optional=) — state= shared algorithm state for the match; resolved is the default.<br>`key` (text, required) — Key matching a declared hashbucket.<br>`result` (text, required) — Joined-row description. | Only valid inside joinstate. | experimental since 1.10.0 | <code>\joinmatch{customer_id}{C42 \(\to\) order/customer row}</code> |
