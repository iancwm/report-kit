...
| Name | Signature | Arguments | Constraints | Stability | Canonical example |
| --- | --- | --- | --- | --- | --- |
| `algorithmtrace` | `O{}` | `options` (options, optional=) — columns= (default 3), columnwidth= and rowheight= in cm (defaults 5.0 and 4.6). | Use inside diagram; it does not open its own tikzpicture.<br>Requires at least one \snapshot; a single snapshot is not a trace. | stable since 1.0.0 | <code>\begin{diagram}[caption={Sliding window advances by one.},description={Three snapshots show the window advancing one step to the right.}]&lt;br&gt;\begin{algorithmtrace}[columns=3]&lt;br&gt;\snapshot{Initial}{\begin{arraystate}\cell{4}\cell{2}\cell{7}\cell{1}\range[state=active]{0}{1}\end{arraystate}}&lt;br&gt;\snapshot{Advance right}{\begin{arraystate}\cell{4}\cell{2}\cell{7}\cell{1}\range[state=active]{1}{2}\end{arraystate}}&lt;br&gt;\snapshot{Advance again}{\begin{arraystate}\cell{4}\cell{2}\cell{7}\cell{1}\range[state=active]{2}{3}\end{arraystate}}&lt;br&gt;\end{algorithmtrace}&lt;br&gt;\end{diagram}</code> |
| `arraystate` | `O{}` | `options` (options, optional=) — rows= (2 or more switches to multi-row \row authoring) and indices= (true shows a muted index row under single-row cells). | Use inside diagram, like reportarchitecture or reportcycle; it does not open its own tikzpicture.<br>With rows unset or 1, author with \cell/\pointer/\range/\annotation. With rows=2 or more, author with \row/\range[row=...]/\annotation instead. | stable since 1.0.0 | <code>\begin{diagram}[caption={Two-pointer scan.},description={A sorted array with left and right pointers bounding the active range.}]&lt;br&gt;\begin{arraystate}&lt;br&gt;\cell{-4}&lt;br&gt;\cell{-1}&lt;br&gt;\cell{-1}&lt;br&gt;\cell{0}&lt;br&gt;\cell{1}&lt;br&gt;\cell{2}&lt;br&gt;\pointer[below]{L}{2}&lt;br&gt;\pointer[below]{R}{5}&lt;br&gt;\range[state=active]{2}{5}&lt;br&gt;\end{arraystate}&lt;br&gt;\end{diagram}</code> |
...
| `annotation` | `m` | `text` (text, required) — The annotation text. | — | stable since 1.0.0 | <code>\annotation{$P_4 - P_1 = 7$}</code> |
...
| `cell` | `O{} m` | `options` (options, optional=) — state= (one of the shared algorithm states; default unseen).<br>`value` (text, required) — The rendered cell value. | Only valid when arraystate's rows key is unset or 1. | stable since 1.0.0 | <code>\cell[state=current]{7}</code> |
...
| `pointer` | `O{} m m` | `options` (options, optional=) — above or below (default below).<br>`label` (text, required) — The pointer's label, e.g. L, R, or mid.<br>`index` (text, required) — The 0-based cell index the pointer marks. | Only valid when arraystate's rows key is unset or 1.<br>Index must be within the declared cells. | stable since 1.0.0 | <code>\pointer[below]{L}{2}</code> |
...
| `range` | `O{} m m` | `options` (options, optional=) — state= (default active) and, in multi-row mode, row= naming a declared \row.<br>`start` (text, required) — 0-based start index, inclusive.<br>`end` (text, required) — 0-based end index, inclusive. | start must not exceed end, and both must lie within the target row's declared cells. | stable since 1.0.0 | <code>\range[state=active]{2}{5}</code> |
...
| `row` | `m m` | `name` (text, required) — The row's name, referenced by \range's row= key and shown as a row label.<br>`values` (text, required) — Comma-separated cell values. | Only valid when arraystate's rows key is 2 or more. | stable since 1.0.0 | <code>\row{values}{3,1,4,2}</code> |
...
| `snapshot` | `m m` | `title` (text, required) — A short label for this step, e.g. "Advance right".<br>`content` (text, required) — The snapshot's own state primitive, e.g. an arraystate. | Only valid inside algorithmtrace. | stable since 1.0.0 | <code>\snapshot{Initial}{\begin{arraystate}\cell{1}\cell{2}\end{arraystate}}</code> |
